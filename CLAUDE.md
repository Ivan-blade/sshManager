...
**下一步候选方向**（用户待定）：① 接入 AI 模型（能力发现接口 `/api/ai/capabilities*` 已就绪，把 LLM 接到 exec/write/buffer）② AI 后台 WebSocket 长连接回显浏览器的「一起干」模式 ③ 会话/分组/快捷命令的拖拽排序 ④ 密码加密存储。

**重要约定**：测试脚本**只清理自己创建的数据**（唯一命名、只删自建），**绝不删除用户数据**（data/*.json 是用户资产）。曾因测试"删除全部"误删用户快捷命令，已约定不再犯。
