...
**已完成并验证**：后端全链路（会话/分组 CRUD、过滤、导入导出、**独立连接模型**——每 tab 一个独立 pty/缓冲、**后台保活**——关页面 SSH 不断可恢复、终端 WS 流 + 增量 buffer、AI 双路径 write/exec/find、SFTP、**AI 能力发现接口**）、前端（**Warp 风深色主题 + xshell 布局 + 分组树 + 多标签终端 + 右键菜单**，双击智能恢复/新建、**关闭 tab 两种方式**、**后台运行标记**，AI 面板已移除）、Electron 壳、pytest 10/10 绿 + 浏览器 E2E 全过 + SSH 真机全链路验证。
