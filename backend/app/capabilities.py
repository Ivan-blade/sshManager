...
CAPABILITIES: list[dict] = [
    ...
    _cap("connect_session", "POST", "/api/sessions/{sid}/connect",
         "建立会话连接（SSH 认证/本地 shell）", [SESSION_PATH], '{"ok":true,"state":"connected"}'),
    _cap("disconnect_session", "POST", "/api/sessions/{sid}/disconnect",
         "断开会话连接", [SESSION_PATH], '{"ok":true,"state":"disconnected"}'),
    _cap("session_status", "GET", "/api/sessions/{sid}/status",
         "查询会话连接状态", [SESSION_PATH], '{"id":..,"connected":bool}'),
    # ---- 分组 ----
    ...
    _cap("delete_group", "DELETE", "/api/groups/{gid}",
         ...
         '{"ok":true}'),
    # ---- AI 执行面 ----
    _cap("write_terminal", "POST", "/api/sessions/{sid}/write",
         "协同路径：向共享终端写入数据（人会看到，输出进缓冲）。适合需要人协作/被观察的场景",
         [SESSION_PATH, {"name": "body", "in": "body", "required": True, "schema": _body_schema(TerminalInput)}],
         '{"ok":true,"written":n}', '{"data":"ls -la\\n"}'),
    _cap("exec_command", "POST", "/api/sessions/{sid}/exec",
         "独立路径：非交互式执行 shell 命令并直接返回结果（不经过共享终端，无并发问题）。默认优先用这个",
    ...
    _cap("read_buffer", "GET", "/api/sessions/{sid}/buffer",
         "增量读取共享终端输出缓冲。since 传上次返回的 total，只取新增内容；gap=true 表示需全量同步",
         [SESSION_PATH, {"name": "since", "in": "query", "type": "integer", "required": False, "default": 0,
...
def overview() -> dict:
    return {
        ...
        "note": "先看概述挑能力，再用 GET /api/ai/capabilities/{name} 查参数；会话类接口需先用 list_sessions 拿 sid。",
